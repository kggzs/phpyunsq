...
$hostid = empty($_GET['hostid'])?exit():intval($_GET['hostid']);
if($qq!=$_GET['qq']) exit;
$host = $DB->get_row("select auth_qq,auth_host,auth_endtime from auth_zzi_authlist where auth_qq = '$qq' and auth_id=".$hostid);
if(!$host){
        exit("<meta http-equiv='Content-Type' content='text/html; charset=utf-8'/>
<script language='javascript'>alert('您的验证信息有误,请刷新页面重试。');history.go(-1);</script>");
}
$authcode = cauthcode($host['auth_host']);

if($conf['down_iswurl'] == 1){
   // showmsg($conf['down_w_url']);
    $html = '<!Doctype html>
<html>
<head>
<meta charset="utf-8">
...
require_once('ayangw/pclzip.php');
if($type=='installer') {  
    $file_path=ROOT.PACKAGE_DIR.'/installer/';
    ...
    $file_path=ROOT.PACKAGE_DIR.'/updater/';
